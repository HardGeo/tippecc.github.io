...
def export_combined_turtle(entity_id: str):
    ...
    ttl_path = Path(dir) / "subgraphs" / f"{entity_id}.ttl"
    json_path = Path(dir) / "subgraphs" / f"{entity_id}.json"

    # Speichern als Turtle
    g.serialize(destination=str(ttl_path), format="ttl")
    # Öffne .ttl-Datei mit UTF-8-Encoding und übergebe keinen Windows-Pfad direkt
    with open(ttl_path, "r", encoding="utf-8") as f:
        graph = prov.read(source=f, format="rdf")

    graph.serialize(destination=str(json_path), format="json")
...
entity_list = [entry.split("/")[-1] for entry in entity_list if not entry.split("/")[-1].startswith("COLLECTION")]
