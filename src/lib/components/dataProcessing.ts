    ...
    while (queue.length > 0) {
        ...
        if (generatedToUsedMap.has(currentEntity)) {
            for (const usedEntity of generatedToUsedMap.get(currentEntity)!) {
                ...
                const variables_used = dataset.entity[usedEntity]["cf:variables"];
                const name_used = variables_used
                .split(", ")  // Split into an array by commas
                .find(part => part.startsWith("name:"))  // Find the segment starting with "name:"
                ?.split(": ")[1];  // Split by ": " and get the value after the colon
                ...
                const unit_used = variables_used
                .split(", ")  // Split into an array by commas
                .find(part => part.startsWith("units:"))  // Find the segment starting with "units:"
                ?.split(": ")[1];  // Split by ": " and get the value after the colon
                ...
                const temporalResolution_used = dataset.entity[usedEntity]["dcat:temporalResolution"];
                ...
                const spatialResolution_used = dataset.entity[usedEntity]["dcat:spatialResolutionInMeters"]["$"];
                ...
                const scenario_used = dataset.entity[usedEntity]["tippecc:scenario"];
                ...
                const format_used = dataset.entity[usedEntity]["datacite:format"];
                ...
                const size_used = dataset.entity[usedEntity]["datacite:size"];
                if (size !== size_used) {
                ...
                const globalmodel_used = dataset.entity[usedEntity]["tippecc:globalmodel"];
                ...
                const regionalmodel_used = dataset.entity[usedEntity]["tippecc:regionalmodel"];
